...
from infrastructure.repositories.teacher_repository_fixed import TeacherRepository
...
class ServiceContainer:
    ...
    def _register_default_services(self) -> None:
        ...
        subject_repository = SubjectRepository(mock_db_manager)
    ...
    async def get(self, name: str) -> BaseService:
        """
    ...
    async def _create_instance(self, name: str) -> BaseService:
        ...
        try:
            ...
            for dep_name in dependencies:
                # Check if this is a repository dependency
                if dep_name.endswith('_repository'):
                    # Create repository instance directly
                    dependency_instances[dep_name] = self._create_repository_instance(dep_name)
                else:
    ...
    def _create_repository_instance(self, repo_name: str):
        """Create repository instance directly."""
        # Get the repository instance from pre-created repositories
        if repo_name in self._repositories:
            return self._repositories[repo_name]
        
        raise KeyError(f"Repository '{repo_name}' not found in pre-created repositories")
