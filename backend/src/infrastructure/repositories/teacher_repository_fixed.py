    ...
    def _get_mock_db(self):
        ...
        class MockDatabase:
            ...
            async def find_one(self, collection, filter):
                ...
                for teacher_id, teacher_data in self._cache.items():
                    if all(teacher_data.get(k) == v for k, v in filter.items()):
                        return teacher_data
                return None
    ...
    async def get_by_id(self, id: str) -> Optional[User]:
        ...
        try:
            ...
            if id in self._cache:
                return User(**self._cache[id])
            
            # Get teacher from database
            teacher = await self._get_by_id(self._collection_name, id)
            
            if teacher:
                # Cache the teacher
                self._cache[id] = teacher
                return User(**teacher)
            
            return None
            
        except Exception as e:
            logger.error(f"Failed to get teacher by ID {id}: {str(e)}")
            raise DatabaseError(f"Failed to get teacher by ID {id}: {str(e)}")
