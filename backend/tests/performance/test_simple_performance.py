...
This module provides basic performance tests that don't rely on complex service containers.
...
class TestSimplePerformance:
    """Simple performance tests for Edu-Flow backend."""
    ...
    def test_database_query_performance(self, mock_db):
        ...
        for i in range(50):
            ...
            mock_db.create_student(student_data)
        
        # Test query performance
        start_time = time.time()
        
        # Query all students
        students = asyncio.run(mock_db.get_all('students', skip=0, limit=50))
        ...
        for student in students:
            if student:
                asyncio.run(mock_db.delete('students', student['id']))
    
    def test_database_update_performance(self, mock_db):
        ...
        for i in range(10):
            ...
            student = asyncio.run(mock_db.create_student(student_data))
        ...
        for i in range(10):
            ...
            asyncio.run(mock_db.update('students', student_id, update_data))
        ...
        # Clean up
        asyncio.run(mock_db.delete('students', student_id))
    
    def test_database_bulk_operations(self, mock_db):
        ...
        for i in range(200):
            if i % 4 == 0:
                ...
                asyncio.run(mock_db.create_student(student_data))
            elif i % 4 == 1:
                # Course creation
                course_data = {
                ...
                asyncio.run(mock_db.create_course(course_data))
            elif i % 4 == 2:
                # Teacher creation
                teacher_data = {
                ...
                asyncio.run(mock_db.create_teacher(teacher_data))
            else:
                # Mark creation
                # First need to get a student and course
                students = asyncio.run(mock_db.get_all('students', skip=0, limit=1))
                courses = asyncio.run(mock_db.get_all('courses', skip=0, limit=1))
                ...
                if students and courses:
                    ...
                    asyncio.run(mock_db.create_mark(mark_data))
        ...
        for entity_type in ['students', 'courses', 'teachers', 'marks']:
            entities = asyncio.run(mock_db.get_all(entity_type))
        ...
        if cleanup_tasks:
            asyncio.run(asyncio.gather(*cleanup_tasks))
